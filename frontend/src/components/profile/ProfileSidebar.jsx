...
const ProfileSidebar = ({ activeSection, setActiveSection }) => {
  ...
  const getIcon = (iconName) => {
    const icons = {
      ...
      bell: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-5 5v-5zM4.558 14.875l.945-1.945A9.97 9.97 0 002 10c0-5.523 4.477-10 10-10s10 4.477 10 10a9.97 9.97 0 00-2.503 6.93l.945 1.945A2 2 0 0122 16H2a2 2 0 01-1.472-3.125z"></path>
        </svg>
      ),
      star: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="w-56 bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 sticky top-24">
      {sidebarSections.map((category, categoryIndex) => (
        <div key={category.category} className={categoryIndex > 0 ? 'mt-6' : 'mt-4'}>
          {/* Category Header - Clickable */}
          <button
            onClick={() => toggleCategory(category.category)}
            className="w-full flex items-center justify-between px-4 py-2 hover:bg-gray-50/50 rounded-lg transition-colors group"
          >
            <div className="flex items-center space-x-2">
              <div className={`bg-gradient-to-r ${category.color} h-1 w-4 rounded-full`}></div>
              ...
              >
                <div className="space-y-1 px-2 mt-3">
                  {category.items.map((item) => (
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        <div className={`flex-shrink-0 ${activeSection === item.id ? 'text-primary' : 'text-gray-400 group-hover:text-primary'}`}>
                          {getIcon(item.icon)}
                        </div>
